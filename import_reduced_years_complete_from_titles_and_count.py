...
try:
    ...
    with connection.cursor() as cursor:
        ...
        sql = "DROP TABLE IF EXISTS `dnb_reduced_years_complete_count`"
        cursor.execute(sql)
    connection.commit()
    print('drop table dnb_reduced_years_complete_count')
    ...
    with connection.cursor() as cursor:
        ...
        sql = "CREATE TABLE IF NOT EXISTS `dnb_reduced_years_complete_count` (`id` MEDIUMINT UNSIGNED NOT NULL AUTO_INCREMENT," \
              " `year` TEXT, count MEDIUMINT UNSIGNED, PRIMARY KEY(id)) " \
              + "ENGINE=InnoDB DEFAULT CHARSET=utf8"
        cursor.execute(sql)
    connection.commit()
    print('create table dnb_reduced_years_complete_count')

    print('open file and read line by line')
    i = 0
    with open('export/years.json') as f:
        data = json.load(f)

        years = []
        for key in util.seq_iter(data):
            years.append([key, data[key]])
        ...
        for y in years:
            ...
            with connection.cursor() as cursor:
                # Create a new record
                i += 1
                sql = "INSERT INTO `dnb_reduced_years_complete_count` (`year`, `count` ) " \
                      "VALUES (%s, %s)"
                try:
