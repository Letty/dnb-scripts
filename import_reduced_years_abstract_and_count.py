...
try:
    ...
    with connection.cursor() as cursor:
        ...
        sql = "DROP TABLE IF EXISTS `dnb_reduced_years_abstract_count`"
        cursor.execute(sql)
    connection.commit()
    print('drop table dnb_reduced_years_abstract_count')
    ...
    with connection.cursor() as cursor:
        ...
        sql = "CREATE TABLE IF NOT EXISTS `dnb_reduced_years_abstract_count` (`id` MEDIUMINT UNSIGNED NOT NULL AUTO_INCREMENT," \
              " `year` TEXT, count MEDIUMINT UNSIGNED, PRIMARY KEY(id)) " \
              + "ENGINE=InnoDB DEFAULT CHARSET=utf8"
        cursor.execute(sql)
    connection.commit()
    print('create table dnb_reduced_years_abstract_count')
    ...
    with open('export/years.json') as f:
        ...
        for y in years:
            ...
            with connection.cursor() as cursor:
                # Create a new record
                i += 1
                sql = "INSERT INTO `dnb_reduced_years_abstract_count` (`year`, `count` ) " \
                      "VALUES (%s, %s)"
                try:
